...
def add_pdb_author_chains(h5file):
    ...
    for pdbid in h5file.keys():
        ...
        pocket_file = h5file[pdbid]['dir'][()].decode('utf-8') + '/' + pdbid + '_pocket.pdb'
...
def add_smiles(h5file):
    ...
    for pdbid in h5file.keys():
        ...
        smiles = p.stdout.read().decode('utf-8').strip().split("converted\n")[1].split()[0]
        print(smiles)
...
data = zip(
    pdbids, 
    [set(h5file[pdbid]['scop2_fams'][()].decode('utf-8').split(',')) for pdbid in pdbids],
...
for pdbid_idx, datum in enumerate(data):
    ...
    same_scop2_fams_bin_idxs = [i for i in range(3) if scop2_fams.intersection(scop2_fams_bins[i])]
...
for i in range(3):
    for j in range(i+1, 3):
        print(f"Bin {i} and {j} have {len(pdbid_bins[i].intersection(pdbid_bins[j]))} pdbids in common.")

# Same for smiles
for i in range(3):
    for j in range(i+1, 3):
        print(f"Bin {i} and {j} have {len(set(smiles_bins[i]).intersection(set(smiles_bins[j])))} smiles in common.")
